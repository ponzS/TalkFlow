  ...
  function cleanupConnections(): void {
    ...
    peerConnections.value.forEach((connection, userId) => {
      try {
        ...
        senders.forEach(sender => {
          if (sender.track) {
            console.log('🛑 停止发送轨道:', sender.track.kind)
            sender.track.stop()
          }
          try {
            connection.removeTrack(sender)
          } catch (error) {
            console.warn('移除发送者失败:', error)
          }
        })
        ...
        receivers.forEach(receiver => {
          if (receiver.track) {
            console.log('🛑 停止接收轨道:', receiver.track.kind)
            receiver.track.stop()
          }
        })
        ...
        // 4. 关闭连接
      connection.close()
        console.log('🔒 已关闭WebRTC连接:', userId.slice(0, 8))
      } catch (error) {
        console.error('清理WebRTC连接时出错:', error)
      }
    })
    ...
    remoteStreams.value.forEach((stream, userId) => {
      stream.getTracks().forEach(track => {
        console.log('🛑 停止远程流轨道:', track.kind, userId.slice(0, 8))
        track.stop()
      })
    })
    ...
    console.log('✅ WebRTC资源清理完成，清理了', audioElements.length, '个音频元素')
  }
