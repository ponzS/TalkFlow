...
export default async function (pwd: string, extra: string) {
...
function arrayBufToBase64UrlEncode(buf: any) {
    return btoa(String.fromCharCode(...buf))
        .replace(/\//g, '_').replace(/=/g, '').replace(/\+/g, '-');
}

function keyBufferToJwk(publicKeyBuffer: any) {
...
async function stretchKey(input: any, salt: any, iterations = 300_000) {
