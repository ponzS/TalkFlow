...
export function useDevChannel() {
  ...
  const loadDevMessages = async (limit: number = 10, append: boolean = false) => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        gun.get('devChannel2Z5w18oB1KhDeBxa4dcutRMCNGFmdpMD_3VxqwVO3KU.R34rQ6Hj4bos0zpdMfe4uyySsFhG9FV49o-pMmr8ZGA').map().once((data: any, messageId: string) => {
          if (data) {
      ...
      await Promise.all(
        sortedMessages.map(async (message) => {
          try {
            await new Promise<void>((resolve) => {
              gun.get('devChannelViews2Z5w18oB1KhDeBxa4dcutRMCNGFmdpMD_3VxqwVO3KU.R34rQ6Hj4bos0zpdMfe4uyySsFhG9FV49o-pMmr8ZGA').get(message.messageId).once((data: any) => {
  ...
  const postDevMessage = async (content: string) => {
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        gun.get('devChannel2Z5w18oB1KhDeBxa4dcutRMCNGFmdpMD_3VxqwVO3KU.R34rQ6Hj4bos0zpdMfe4uyySsFhG9FV49o-pMmr8ZGA').get(message.messageId).put(message, (ack: any) => {
  ...
  const deleteDevMessage = async (messageId: string) => {
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        gun.get('devChannel2Z5w18oB1KhDeBxa4dcutRMCNGFmdpMD_3VxqwVO3KU.R34rQ6Hj4bos0zpdMfe4uyySsFhG9FV49o-pMmr8ZGA').get(messageId).put(null, (ack: any) => {
          if (ack.err) reject(new Error(ack.err));
          else resolve();
        });
      });
  ...
  const incrementViewCount = async () => {
    try {
      await new Promise<void>((resolve) => {
        gun.get('devChannelTotalViews2Z5w18oB1KhDeBxa4dcutRMCNGFmdpMD_3VxqwVO3KU.R34rQ6Hj4bos0zpdMfe4uyySsFhG9FV49o-pMmr8ZGA').once((data: any) => {
        ...
        try {
          await new Promise<void>((resolve) => {
            gun.get('devChannelViews2Z5w18oB1KhDeBxa4dcutRMCNGFmdpMD_3VxqwVO3KU.R34rQ6Hj4bos0zpdMfe4uyySsFhG9FV49o-pMmr8ZGA').get(message.messageId).once((data: any) => {
              ...
              gun.get('devChannelViews2Z5w18oB1KhDeBxa4dcutRMCNGFmdpMD_3VxqwVO3KU.R34rQ6Hj4bos0zpdMfe4uyySsFhG9FV49o-pMmr8ZGA').get(message.messageId).put({ count: currentCount + 1 }, (ack: any) => {
  ...
  const listenTotalViews = () => {
    gun.get('devChannelTotalViews2Z5w18oB1KhDeBxa4dcutRMCNGFmdpMD_3VxqwVO3KU.R34rQ6Hj4bos0zpdMfe4uyySsFhG9FV49o-pMmr8ZGA').on((data: any) => {
