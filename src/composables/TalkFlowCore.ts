...
// // async function performLogin(pair: any, encryptedKey: string, alias: string): Promise<void> {
// //   try {
// //     await new Promise<void>((resolve, reject) => {
// //       user.auth(pair, async (ack: any) => {
// //         if (ack.err) {
...
// //           isLoggedIn.value = true;
// //           currentUserPub.value = pair.pub;
// //           currentUserPair.value = pair;

// //           let userData = await storageServ.getUser(pair.pub);
// //           if (!userData) {
// //             await storageServ.saveUser(pair.pub, alias || 'DefaultUser', undefined, encryptedKey);
// //             userData = await storageServ.getUser(pair.pub);
// //           }
// //           currentUserAlias.value = userData!.alias || '';
// //           userAvatars.value[pair.pub] = userData!.avatar || '';
...
// // async function generateKeyPairFaceId(): Promise<void> {
// //   const alias = newAlias.value.trim();
// //   const pass = newPassphrase.value;
...
// //       ]);
// //       if (!pair || !pair.pub || !pair.priv || !pair.epub || !pair.epriv) {
// //         throw new Error('SEA.pair The necessary fields are missing, please try again.');
// //       }
// //       if (!existingPubKeys.has(pair.pub)) {
// //         unique = true;
// //       } else {
...
// //   encryptedKeyPair.value = encrypted;
// //   await storageServ.saveUser(pair.pub, alias, undefined, encrypted);
// //   generateMsg.value = 'The key is successfully generated.';
// //   showToast(generateMsg.value, 'success');
// //   KeyDown.value = true;

// //   // Verify Face ID before saving to secure storage
// //   try {
// //     await BiometricAuth.authenticate({
// //       reason: 'Save your credentials securely with Face ID',
// //       cancelTitle: 'Cancel',
// //     });
// //     await SecureStorage.setItem('userPassphrase', pass);
// //     await SecureStorage.setItem('userEncryptedKey', encrypted);
// //     showToast('Credentials saved securely with Face ID', 'success');

// //     // Auto-login after saving
// //     passphrase.value = pass;
// //     encryptedKeyInput.value = encrypted;
// //     await performLogin(pair, encrypted, alias);
// //   } catch (err) {
// //     generateMsg.value = 'Failed to authenticate with Face ID or save credentials';
// //     showToast(generateMsg.value, 'error');
// //     console.error('Biometric auth or secure storage error:', err);
// //   }
...
// // async function importKeyPairFaceId(): Promise<void> {
// //   try {
// //     // Prompt for Face ID authentication
// //     await BiometricAuth.authenticate({
// //       reason: 'Access your credentials with Face ID',
// //       cancelTitle: 'Cancel',
// //     });

// //     // Retrieve credentials from secure storage
// //     const passphraseStored = await SecureStorage.getItem('userPassphrase');
// //     const encryptedKeyStored = await SecureStorage.getItem('userEncryptedKey');

// //     if (!passphraseStored || !encryptedKeyStored) {
// //       loginError.value = 'No credentials found in secure storage';
// //       showToast(loginError.value, 'warning');
// //       return;
// //     }

// //     // Decrypt the key pair
// //     const decrypted = decryptData(encryptedKeyStored, passphraseStored);
// //     if (!decrypted) {
// //       loginError.value = 'Decryption failure or incorrect password';
// //       showToast(loginError.value, 'error');
// //       return;
...
// //       pair = JSON.parse(decrypted);
// //     } catch (e) {
// //       loginError.value = 'Decryption successful, but the key format is wrong';
// //       showToast(loginError.value, 'error');
// //       return;
// //     }
...
// //     await performLogin(pair, encryptedKeyStored, pair.alias || 'No Name');
// //   } catch (err) {
// //     loginError.value = 'Face ID authentication failed or credentials not found';
// //     showToast(loginError.value, 'error');
// //     console.error('Biometric auth or secure storage error:', err);
// //   }
// // }
...
// /**
//  * 从本地数据库加载并解密用户密钥对
//  * @returns 完整的密钥对对象（KeyPair）或 null（失败时）
//  */
// async function getUserKeyPair(): Promise<KeyPair | null> {
//   try {
//     // 1. 从本地数据库加载凭据
//     const credentialsResult = await storageServ.query('SELECT value FROM credentials WHERE key = ?', ['userCredentials']);
//     if (!credentialsResult.values || !credentialsResult.values.length) {
//       console.log('未找到本地凭据，无法获取密钥对');
//       showToast('未找到登录凭证，请手动登录', 'info');
//       return null;
//     }
...
//     const decryptedCredentials = decryptData(encryptedCredentials, 'talkflow-secret-key');
//     if (!decryptedCredentials) {
//       console.log('凭据解密失败，无法获取密钥对');
//       showToast('无法解密登录凭证，请手动登录', 'error');
//       return null;
//     }
...
//     const decrypted = decryptData(storedKeyPair, storedPass);
//     if (!decrypted) {
//       console.log('密钥对解密失败，无法获取密钥对');
//       showToast('密钥对解密失败，请手动登录', 'error');
//       return null;
//     }
...
//     return pair;
//   } catch (error) {
//     console.error('获取用户密钥对失败:', error);
//     showToast('无法获取密钥对，请手动登录', 'error');
//     return null;
//   }
// }
