            ...
            <div class="meta-text-container" :class="selectedMsgObj.sender === safeUserAlias ? 'my-message' : 'other-message'">
              ...
              >
                ...
                <template v-else-if="isBase64Audio(seg.content)">
                  <ion-button @click.stop="toggleVoicePlayback(seg.content)">Play</ion-button>
                </template>
          ...
          >
            ...
            >
                ...
                >
                  <template v-if="item.text">
                    ...
                    <template v-else-if="isBase64Audio(item.text)">
                      <ion-button @click.stop="toggleVoicePlayback(item)">Play</ion-button>
                    </template>
              ...
              >
                ...
                />
                <div 
                  v-else-if="item.status === 'sent' && item.justSent && item.sender === safeUserAlias"
      ...
      <ion-toolbar class="input-toolbar" :style="{ transform: `translateY(-${keyboardHeight}px)` }">
        <div class="input-capsule" ref="capsuleRef" :class="{ 'shift-up': isDrawerOpen }">
          ...
          <div class="input-container">
            <ion-button class="mode-toggle" fill="clear" @click="toggleInputMode">
              <ion-icon slot="icon-only" :icon="isVoiceMode ? chatbubbleEllipsesOutline : micOutline"></ion-icon>
            </ion-button>
            ...
            >
              <span v-if="voiceBar.isRecording.value">{{ cancelRecording ? '滑动取消' : '录音中 ' + voiceBar.formatDuration(voiceBar.recordingDuration.value) }}</span>
              <span v-else>按住录音</span>
            </div>
          </div>
        </div>
      </ion-toolbar>
...
async function loadOlderMessages() {
  ...
  const prevHeight = scrollerRef.value.$el.scrollHeight;
  const prevScrollTop = scrollerRef.value.$el.scrollTop;
  ...
  nextTick(() => {
    if (scrollerRef.value) {
      ...
      scrollerRef.value.$el.scrollTop = prevScrollTop + (newHeight - prevHeight);
      scrollerRef.value.reset();
    }
  });
...
function onScrollerScroll(event: Event) {
  const target = event.target as HTMLElement;
  if (target.scrollTop < 100 && !isLoadingOlderMessages.value && hasMoreMessages.value) {
    loadOlderMessages();
  }
}
