...
export class NavigationStateService {
  ...
  async saveNavigationState(userPub: string, currentComponent: string, activeTab: string): Promise<void> {
    try {
      ...
      console.log(`导航状态已保存: ${userPub} -> ${currentComponent} (${activeTab})`);
    } catch (error) {
  ...
  async getNavigationState(userPub: string): Promise<NavigationState | null> {
    try {
      ...
      if (userState) {
        console.log(`恢复导航状态: ${userPub} -> ${userState.currentComponent} (${userState.activeTab})`);
        return userState;
      }
  ...
  async removeNavigationState(userPub: string): Promise<void> {
    try {
      ...
      if (existingData.navigationStates[userPub]) {
        ...
        console.log(`导航状态已删除: ${userPub}`);
      }
    } catch (error) {
      console.warn('删除导航状态失败:', error);
    }
  }
  ...
  async clearAllNavigationStates(): Promise<void> {
    try {
      ...
      console.log('所有导航状态已清空');
    } catch (error) {
      console.warn('清空导航状态失败:', error);
      throw error;
    }
  }
  ...
  private async loadNavigationStates(): Promise<NavigationStatesData> {
    try {
      ...
      // 文件不存在或格式错误，返回默认结构
      console.log('导航状态文件不存在或损坏，创建新文件');
      ...
      try {
        ...
        console.warn('创建导航状态文件失败:', writeError);
      }
