...
export class SegmentStateService {
  ...
  async saveSegmentState(pageId: string, selectedSegment: string): Promise<void> {
    try {
      ...
      console.log(`Segment 状态已保存: ${pageId} -> ${selectedSegment}`);
    } catch (error) {
      console.warn('保存 segment 状态失败:', error);
      throw error;
    }
  }
  ...
  async getSegmentState(pageId: string): Promise<SegmentState | null> {
    try {
      ...
      if (pageState) {
        console.log(`恢复 segment 状态: ${pageId} -> ${pageState.selectedSegment}`);
        return pageState;
      }
      
      return null;
    } catch (error) {
      console.warn('获取 segment 状态失败:', error);
      return null;
    }
  }
  ...
  async removeSegmentState(pageId: string): Promise<void> {
    try {
      ...
      if (existingData.segmentStates[pageId]) {
        ...
        console.log(`Segment 状态已删除: ${pageId}`);
      }
    } catch (error) {
      console.warn('删除 segment 状态失败:', error);
    }
  }
  ...
  async clearAllSegmentStates(): Promise<void> {
    try {
      ...
      console.log('所有 segment 状态已清空');
    } catch (error) {
      console.warn('清空 segment 状态失败:', error);
      throw error;
    }
  }
  ...
  private async loadSegmentStates(): Promise<SegmentStatesData> {
    try {
      ...
      // 文件不存在或格式错误，返回默认结构
      console.log('Segment 状态文件不存在或损坏，创建新文件');
      ...
      try {
        ...
        console.warn('创建 segment 状态文件失败:', writeError);
      }
