...
export class GlobalPersistentMessageQueue {
  ...
  private triggerStatusCallback(messageId: string, status: 'sending' | 'sent' | 'failed'): void {
    ...
    if (callback) {
      try {
        ...
        console.error(`Error in status callback for ${messageId}:`, error)
      }
    }
  }
  ...
  async enqueue(message: QueuedMessage): Promise<void> {
    ...
    console.log(`📤 Message queued globally: ${message.id} (retry: ${message.retryCount})`)
  ...
  async updateRetry(messageId: string, retryCount: number, error?: string): Promise<void> {
    ...
    console.log(`🔄 Message retry updated globally: ${messageId} (count: ${retryCount}, next: ${delay}ms)`)
  ...
  private async processQueue(): Promise<void> {
    ...
    try {
      ...
      console.log(`🚀 Parallel processing ${readyMessages.length} messages (${this.activeSends.size} currently active)...`)
  ...
  private async sendMessage(queuedMessage: QueuedMessage): Promise<void> {
    ...
    try {
      console.log(`🚀 Sending message in parallel: ${messageId} (attempt: ${queuedMessage.retryCount + 1}, active: ${this.activeSends.size})`)
      ...
      console.log(`✅ Message sent successfully in parallel: ${messageId} (remaining active: ${this.activeSends.size - 1})`)
  ...
  private startBackgroundProcessing(): void {
    ...
    this.networkCheckInterval = setInterval(async () => {
      if (navigator.onLine) {
        const stats = await this.getQueueStats()
        if (stats.total > 0) {
          console.log(`🌐 Network available, ${stats.total} queued messages (${stats.activeSending} currently sending)...`)
          this.processQueue()
        }
      }
    ...
    window.addEventListener('online', async () => {
      const stats = await this.getQueueStats()
      console.log(`🌐 Network restored! ${stats.total} messages in queue, starting parallel processing...`)
    ...
    window.addEventListener('offline', () => {
      console.log('📴 Network lost, messages will continue queuing for parallel sending when restored')
    })
    ...
    document.addEventListener('visibilitychange', async () => {
      if (!document.hidden) {
        const stats = await this.getQueueStats()
        console.log(`👀 Page visible, ${stats.total} messages in queue (${stats.activeSending} active), processing...`)
        this.processQueue()
      }
    })
  }
