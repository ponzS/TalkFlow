...
const navigateToProfile = async (pub: string) => {
  ...
  onMounted(() => {
    ...
    if (hasCapacitor) {
      ...
      keyboardShowListener = Keyboard.addListener('keyboardWillShow', (info: KeyboardInfo) => {
...
onUnmounted(() => {
  ...
  if (hasCapacitor) {
    ...
    console.log('🔧 TenMinutes: Removed only own keyboard listeners');
  }
});
</script>
